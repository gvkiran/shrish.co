...
// Signs unsubscribe links so nobody can opt somebody else out.
...
/* ═══════════════ MARKETING CAMPAIGNS ═══════════════
 *
 * Sent from a separate subdomain so a bad campaign cannot damage the sending
 * reputation of order confirmations and tracking emails. Those are the emails
 * the business actually depends on; marketing is optional and must never be
 * able to break them.
 *
 * Unsubscribe is not decoration. CAN-SPAM requires a working opt-out and a
 * postal address, and every send checks the suppression list first.
 */

const MARKETING_FROM_EMAIL = "Shrish <hello@news.shrish.co>";
const SHRISH_POSTAL_ADDRESS = "Chesterfield, VA, USA";
const CAMPAIGN_BATCH_LIMIT = 120;

function emailKey(email) {
  return crypto.createHash("sha256").update(String(email).trim().toLowerCase()).digest("hex").slice(0, 40);
}

// Stateless token: nothing to store at send time, and it cannot be guessed
// without the secret, so no one can unsubscribe somebody else.
...
function unsubscribeUrl(email, secret) {
  const encoded = Buffer.from(String(email).trim().toLowerCase()).toString("base64url");
  return `https://us-central1-shrish-website.cloudfunctions.net/unsubscribeEmail`
    + `?e=${encodeURIComponent(encoded)}&t=${encodeURIComponent(unsubscribeToken(email, secret))}`;
}

function campaignFooter(email, secret) {
  return `<div style="background:#f6f1e8; padding:16px 24px; font-size:11px; color:#7a6853; line-height:1.7; text-align:center;">
      You are receiving this because you ordered from Shrish.<br />
      <a href="${unsubscribeUrl(email, secret)}" style="color:#7a6853;">Unsubscribe from occasional emails</a>
      &nbsp;·&nbsp; Shrish LLC, ${escapeHtml(SHRISH_POSTAL_ADDRESS)}
    </div>`;
}

function campaignShell(headline, bodyHtml, email, secret) {
  ...
  <div style="padding:28px 12px;">
    <div style="max-width:640px;margin:0 auto;background:#fff;border-radius:18px;overflow:hidden;">
      <div style="background:#b87512;padding:26px 24px;text-align:center;">
        <img src="${SHRISH_LOGO_URL}" alt="Shrish" width="96" style="display:block;margin:0 auto 12px;" />
...
const CTA = (url, label) =>
  `<div style="text-align:center;margin:0 0 22px;"><a href="${url}" style="display:inline-block;background:#b87512;color:#fff;text-decoration:none;font-weight:700;font-size:15px;padding:13px 34px;border-radius:50px;">${escapeHtml(label)}</a></div>`;

function buildCampaignEmail(template, fields, recipient, secret) {
  const first = escapeHtml(recipient.firstName || "there");
  const sign = `<p style="margin:18px 0 0;font-size:15px;line-height:1.7;">— Kiran, Shrish</p>`;

  if (template === "checkin") {
    const product = recipient.lastProduct
      ? ` We hope the ${escapeHtml(recipient.lastProduct)} was everything you hoped for.`
      : "";
    return campaignShell("We hope you enjoyed it", `
      <p style="margin:0 0 16px;font-size:15px;line-height:1.7;">Hi ${first},</p>
      <p style="margin:0 0 16px;font-size:15px;line-height:1.7;">
        It has been a little while since your last order, so we wanted to check in.${product}
      </p>
      ...
      <div style="background:#fff8ec;border:1px solid #ecd9b6;border-radius:14px;padding:20px 18px;text-align:center;margin-bottom:20px;">
        ...
        <a href="${SHRISH_REVIEW_URL}" style="display:inline-block;background:#b87512;color:#fff;text-decoration:none;font-weight:700;font-size:14px;padding:12px 28px;border-radius:50px;">Leave a review</a>
      </div>
      ...
      </p>${sign}`, recipient.email, secret);
  }

  if (template === "promo") {
    return campaignShell(String(fields.headline || "A little something off your next order"), `
      <p style="margin:0 0 16px;font-size:15px;line-height:1.7;">Hi ${first},</p>
      <p style="margin:0 0 20px;font-size:15px;line-height:1.7;">${escapeHtml(fields.intro || "")}</p>
      <div style="background:#fff8ec;border:2px dashed #d9a441;border-radius:14px;padding:22px 18px;text-align:center;margin-bottom:20px;">
        ...
        <div style="font-size:30px;font-weight:800;letter-spacing:2px;color:#8a5a12;margin:6px 0 4px;">${escapeHtml(fields.promoCode || "")}</div>
        <div style="font-size:13px;color:#6b5842;">${escapeHtml(fields.offerLine || "")}</div>
      </div>
      ${CTA("https://www.shrish.co/shop.html", "Shop now")}
      <p style="margin:0;font-size:13px;line-height:1.7;color:#6b5842;">${escapeHtml(fields.terms || "")}</p>${sign}`,
      recipient.email, secret);
  }

  // announce — new product, combo pack, festival offer
  return campaignShell(String(fields.headline || "Something new at Shrish"), `
    <p style="margin:0 0 16px;font-size:15px;line-height:1.7;">Hi ${first},</p>
    <p style="margin:0 0 20px;font-size:15px;line-height:1.7;">${escapeHtml(fields.intro || "")}</p>
    <div style="border:1px solid #ecd9b6;border-radius:14px;overflow:hidden;margin-bottom:20px;">
      ${fields.imageUrl ? `<img src="${escapeHtml(fields.imageUrl)}" alt="${escapeHtml(fields.productName || "")}" style="display:block;width:100%;height:auto;" />` : ""}
      <div style="padding:18px;">
        <div style="font-size:17px;font-weight:700;">${escapeHtml(fields.productName || "")}</div>
        <div style="font-size:14px;line-height:1.7;color:#3d3225;margin-top:6px;">${escapeHtml(fields.productDescription || "")}</div>
        ${fields.price ? `<div style="font-size:18px;font-weight:700;color:#8a5a12;margin-top:10px;">${escapeHtml(fields.price)}</div>` : ""}
      </div>
    </div>
    ${CTA(fields.productUrl || "https://www.shrish.co/shop.html", "Have a look")}
    <p style="margin:0;font-size:14px;line-height:1.7;color:#6b5842;">${escapeHtml(fields.closing || "")}</p>${sign}`,
    recipient.email, secret);
}

// Public opt-out endpoint. Serves its own confirmation page so the customer
// sees something reassuring rather than raw JSON.
exports.unsubscribeEmail = onRequest(
  { region: "us-central1", secrets: [UNSUBSCRIBE_SECRET] },
  async (request, response) => {
    const page = (title, message) => `<!DOCTYPE html><html><head><meta charset="utf-8" />
      <meta name="viewport" content="width=device-width,initial-scale=1" /><title>${title}</title></head>
      <body style="margin:0;background:#ece7df;font-family:Arial,Helvetica,sans-serif;color:#2b2218;">
      <div style="max-width:520px;margin:60px auto;background:#fff;border-radius:18px;padding:34px 28px;text-align:center;">
        <img src="${SHRISH_LOGO_URL}" alt="Shrish" width="80" style="display:block;margin:0 auto 18px;" />
        <h1 style="font-size:21px;margin:0 0 10px;">${title}</h1>
        <p style="font-size:15px;line-height:1.7;color:#5a4a38;margin:0;">${message}</p>
      </div></body></html>`;

    try {
      const encoded = String(request.query.e || "");
      const token = String(request.query.t || "");
      if (!encoded || !token) {
        response.status(400).send(page("Link not valid", "This unsubscribe link is incomplete."));
        return;
      }

      const email = Buffer.from(encoded, "base64url").toString("utf8").trim().toLowerCase();
      const expected = unsubscribeToken(email, normalizedSecret(UNSUBSCRIBE_SECRET));
      if (token !== expected) {
        response.status(403).send(page("Link not valid", "This unsubscribe link could not be verified."));
        return;
      }

      await admin.firestore().collection("email_optouts").doc(emailKey(email)).set({
        email,
      ...
      response.status(200).send(page(
        ...
        "You will not receive any more marketing emails from us. Order confirmations and delivery updates will still be sent, because you need those. Thank you for having supported our small kitchen."
      ));
    } catch (error) {
      console.error("Unsubscribe failed", error);
      response.status(500).send(page("Something went wrong", "Please email contact@shrish.co and we will remove you by hand."));
    }
  }
);

// Admin-only campaign send. Suppression is checked here, on the server, so it
// cannot be bypassed by anything happening in the browser.
exports.sendCampaign = onCall(
  callableOptions({ secrets: [RESEND_API_KEY, UNSUBSCRIBE_SECRET] }),
  async (request) => {
    if (!isAdminRequest(request)) {
      throw new HttpsError("permission-denied", "Admin access is required.");
    }

    const template = String(request.data?.template || "").trim();
    if (!["checkin", "promo", "announce"].includes(template)) {
      throw new HttpsError("invalid-argument", "Unknown template.");
    }

    const subject = String(request.data?.subject || "").trim().slice(0, 160);
    if (!subject) throw new HttpsError("invalid-argument", "A subject line is required.");
    ...
    if (!rawRecipients.length) throw new HttpsError("invalid-argument", "No recipients.");
    if (rawRecipients.length > CAMPAIGN_BATCH_LIMIT) {
      throw new HttpsError("invalid-argument", `Send ${CAMPAIGN_BATCH_LIMIT} or fewer at a time.`);
    }

    const db = admin.firestore();
    const secret = normalizedSecret(UNSUBSCRIBE_SECRET);
    if (!secret) throw new HttpsError("failed-precondition", "UNSUBSCRIBE_SECRET is not configured.");

    // De-duplicate, then drop anyone who has opted out.
    ...
    for (const entry of rawRecipients) {
      ...
      if (!byEmail.has(email)) {
        byEmail.set(email, {
          email,
          firstName: String(entry?.firstName || "").trim().slice(0, 60),
          lastProduct: String(entry?.lastProduct || "").trim().slice(0, 80),
        });
      }
    }

    const optOutSnap = await db.collection("email_optouts").get();
    const suppressed = new Set(optOutSnap.docs.map((doc) => String(doc.data()?.email || "").toLowerCase()));

    const recipients = [...byEmail.values()].filter((entry) => !suppressed.has(entry.email));
    ...
    for (const recipient of recipients) {
      try {
        await resend.emails.send({
          ...
          to: [recipient.email],
          subject,
          html: buildCampaignEmail(template, fields, recipient, secret),
          headers: {
            // Gmail and Outlook surface a native one-click unsubscribe from
            // these, which materially reduces spam complaints.
            "List-Unsubscribe": `<${unsubscribeUrl(recipient.email, secret)}>`,
            "List-Unsubscribe-Post": "List-Unsubscribe=One-Click",
          },
        });
        sent += 1;
      } catch (error) {
        console.error("Campaign send failed", { email: recipient.email, message: error?.message });
    ...
    await campaignRef.set({
      ...
      subject,
      fields,
      attempted: byEmail.size,
      sent,
      skippedOptOut: skipped,
      failed: failures.length,
      failedEmails: failures.slice(0, 50),
      sentBy: request.auth?.token?.email || "admin",
      sentAt: admin.firestore.FieldValue.serverTimestamp(),
    });

    return { sent, skipped, failed: failures.length, campaignId: campaignRef.id };
  }
);
