...
function addToCart(productId, qty, variantId = null) {
  ...
  saveCart();
  updateCartUI();
  showToast(`Added ${selectedVariant.id === 'default' ? p.name : `${p.name} (${selectedVariant.label})`}!`);
...
function selectedVariantDisplay(product, variantId = null) {
...
function openModal(productId) {
  ...
  const selectedDisplay = selectedVariantDisplay(p, modalVariantId);
  ...
  if (mainWrap) {
    mainWrap.innerHTML = imgs.length
      ...
      : `<div class="modal-img-placeholder">Mango</div>`;
  }
...
function renderCard(p) {
  ...
  const primaryDisplay = selectedVariantDisplay(p, variants[0]?.id);
  ...
  return `<div class="pc ${isSoon ? 'display-only' : ''} ${!isAvail && !isSoon ? 'sold-out' : ''}">
      ...
      <div class="pc-img" onclick="openModal('${escapeHtml(p.id)}')">
        ${imgHtml}
        <div class="pc-img-emoji" ${emojiStyle}>Mango</div>
        <div class="pc-status-strip ${stripCls}">${escapeHtml(stripText)}</div>
      ...
      <div class="pc-body">
        ...
        <div class="pc-footer"><div class="pc-price-wrap"><div class="pc-price">${escapeHtml(primaryDisplay.price)}</div><div class="pc-unit">${escapeHtml(primaryDisplay.unit)}</div></div></div>
        ${actionHtml}
      </div>
    </div>`;
}
