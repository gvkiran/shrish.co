...
function relatedHtml(product, products) {
  ...
  const siblings = products
    .filter((p) => normalizeCategory(p.category) === category && p.id !== product.id)
...
function breadcrumbLd(product) {
  const category = normalizeCategory(product.category);
  return JSON.stringify({
    '@context': 'https://schema.org',
    '@type': 'BreadcrumbList',
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: 'Shop', item: `${SITE_URL}/shop.html` },
      { '@type': 'ListItem', position: 2, name: CATEGORY_LABELS[category] || category, item: `${SITE_URL}/shop/products/` },
      { '@type': 'ListItem', position: 3, name: product.name, item: productPageUrl(product) }
    ]
  }, null, 2).replace(/</g, '\\u003c');
}
...
<head>
  ...
  <style>
    ...
    .product-related h2 { font-family: var(--font-display); color: var(--dark); font-size: 24px; margin-bottom: 12px; }
    .related-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 10px; }
    .related-grid a { background: var(--white); border: 1px solid rgba(200,121,26,.14); border-radius: 10px; padding: 11px 13px; color: var(--text); font-weight: 700; font-size: 14px; }
    .related-grid a:hover { color: var(--saffron); }
    .related-all { display: inline-block; margin-top: 12px; color: var(--saffron); font-weight: 700; font-size: 14px; }
  ...
  <script type="application/ld+json">${breadcrumbLd(product)}</script>
</head>
...
function updateShopLinks(products) {
  const shopPath = path.join(ROOT, 'shop.html');
  let html = fs.readFileSync(shopPath, 'utf8');
  const START = '<!-- PRODUCT-LINKS:START auto-generated by scripts/generate-product-pages.js; do not edit by hand -->';
  const END = '<!-- PRODUCT-LINKS:END -->';
  const grouped = products.reduce((acc, product) => {
    const category = normalizeCategory(product.category);
    (acc[category] = acc[category] || []).push(product);
    return acc;
  }, {});
  const sections = Object.entries(grouped).map(([category, items]) => `      <div class="seo-links-group">
        <h3>${escapeHtml(CATEGORY_LABELS[category] || category)}</h3>
        <ul>
          ${items.map((p) => `<li><a href="shop/products/${category}/${p.id}/">${escapeHtml(p.name)}</a></li>`).join('\n          ')}
        </ul>
      </div>`).join('\n');
  const block = `${START}
<section class="seo-product-links" aria-label="Browse all products">
  <style>
    .seo-product-links { border-top: 1px solid rgba(200,121,26,.18); padding: 26px 24px 34px; }
    .seo-product-links-inner { max-width: 1240px; margin: 0 auto; }
    .seo-product-links summary { cursor: pointer; list-style: none; display: flex; align-items: center; gap: 10px; font-family: var(--font-display, serif); font-size: 20px; color: #D9A24A; }
    .seo-product-links summary::-webkit-details-marker { display: none; }
    .seo-product-links summary::after { content: '+'; font-size: 22px; color: #D9A24A; transition: transform .2s; }
    .seo-product-links details[open] summary::after { transform: rotate(45deg); }
    .seo-links-columns { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px; padding-top: 18px; }
    .seo-links-group h3 { color: #D9A24A; font-size: 12px; margin: 0 0 8px; text-transform: uppercase; letter-spacing: .12em; }
    .seo-links-group ul { list-style: none; margin: 0; padding: 0; }
    .seo-links-group li { margin: 0 0 6px; }
    .seo-links-group a { color: rgba(253,246,236,.72); font-size: 13px; text-decoration: none; }
    .seo-links-group a:hover { color: #D9A24A; }
    .seo-links-directory { display: inline-block; margin-top: 14px; color: #D9A24A; font-weight: 700; font-size: 13px; }
  </style>
  <div class="seo-product-links-inner">
    <details>
      <summary>Browse the full catalog</summary>
      <div class="seo-links-columns">
${sections}
      </div>
      <a class="seo-links-directory" href="shop/products/">View the full product directory &rarr;</a>
    </details>
  </div>
</section>
${END}`;
  ...
  if (startIdx !== -1) {
    ...
    if (endIdx === -1) throw new Error('PRODUCT-LINKS END marker missing in shop.html');
    html = html.slice(0, startIdx) + block + html.slice(endIdx + END.length);
  } else {
    const footerTag = '<footer class="footer">';
    if (!html.includes(footerTag)) throw new Error('Could not find footer in shop.html to insert product links');
    html = html.replace(footerTag, block + '\n\n' + footerTag);
  }
  fs.writeFileSync(shopPath, html);
  console.log('Injected crawlable product links into shop.html');
}
...
function writeProductPages() {
  ...
  for (const product of products) {
    ...
    fs.writeFileSync(path.join(dir, 'index.html'), renderProductPage(product, products));
  }
